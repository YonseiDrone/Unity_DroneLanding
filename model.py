...
class Actor(tf.keras.Model):
    def __init__(self, action_dim):
        ...
        self.dense1_layer = tf.keras.layers.Dense(64, activation=tf.nn.relu)
        self.dense2_layer = tf.keras.layers.Dense(64, activation=tf.nn.relu)
        self.dense3_layer = tf.keras.layers.Dense(64, activation=tf.nn.relu)
    ...
    def call(self, state):
        ...
        a3 = self.dense3_layer(a2)
        mu = self.mean_layer(a3)
...
class Critic(tf.keras.Model):
    ...
    def call(self, state, action):
        ...
        a2 = self.dense2_layer(a1)
        a3 = self.dense2_layer(a2)
        q = self.output_layer(a3)
        return q
